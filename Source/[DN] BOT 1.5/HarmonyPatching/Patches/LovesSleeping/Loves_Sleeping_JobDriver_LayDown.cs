...
{
    public class Loves_Sleeping_JobDriver_LayDown
    {
        private const float ExtraMult = 0.00002f;

        public static IEnumerable<Toil> MakeNewToils_PostFix(IEnumerable<Toil> values, JobDriver_LayDown ___instance)
        {
            foreach (Toil toil in values) 
            {
                if (toil.debugName == "LayDown" && toil.actor.HasTrait(BOT_TraitDefOf.BOT_Loves_Sleeping))
                {
                    void tickAction()
                    {
        ...
        //public static void Postfix(Toil __result, TargetIndex bedOrRestSpotIndex, bool hasBed, bool canSleep = true, bool gainRestAndHealth = true, PawnPosture _ = PawnPosture.LayingOnGroundNormal)
        //{
